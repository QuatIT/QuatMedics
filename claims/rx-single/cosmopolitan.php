...
$patient_details = select("select * from insure_data where patientid='$patient_id' && dateinsert='$date_insert' && insurancetype='$insurance' ");
...
$consultation_query = select("select * from consultation where patientID='$patient_id' AND insuranceType='$insurance' AND dateInsert='$date_insert' ");
foreach($consultation_query as $consultation_row){}

//check patient table for patient detail
$patient_details = select("select * from patient where patientID='$patient_id' ");
foreach($patient_details as $patient_row){}

//Check diagnosis
$diagnosis_query = select("select * from diagnose_tb where patientID='$patient_id' && dateRegistered='$date_insert' ");

//CHECK TARIFF IN NHIS_TARIFF
$med_tarif = select("select * from nhis_tariff where patientid='$patient_id'");
foreach($med_tarif as $med_taf){}

//check medicine for tariff
$med_tarf = select("select * from insure_med_frequency where patientid='$patient_id' && dateinsert='$date_insert' ");
          ...
          <tbody>
            ...
            <?php foreach($med_tarf as $medt){ ?>
              <tr>
                ...
                <td style="text-align:center"><?php echo count(select("select count(medicine) from insure_med_frequency where patientid='$patient_id' && dateinsert='$date_insert' ")); ?></td>
