...
if(isset($_GET['lid'])){
   $lid = $_GET['lid'];
    //fecth lab details..
    $fetchLab = select("SELECT * FROM lablist WHERE labID='$lid' AND centerID='$centerID'");
    if($fetchLab){
        foreach($fetchLab as $flabrow){}
    }
}else{
    $error = "<script>document.write('NO LAB SELECTED.');</script>";
}

//update labcodes..
if(isset($_POST['updatelab'])){
    $labID = trim(htmlentities($_POST['labID']));
    $labName = trim(htmlentities($_POST['labName']));
    //update labtest
    $updateLab = update("UPDATE lablist SET labName='$labName' WHERE labID='$labID'");
    if($updateLab){
        $success = "<script>document.write('LAB UPDATED SUCCESSFUL.');window.location.href='centerlab-index';</script>";
    }else{
        $error = "<script>document.write('LAB UPDATE FAILED, TRY AGAIN.');</script>";
    }
}
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb">
        ...
        <a title="Lab Management" class="tip-bottom"><i class="fa fa-edit"></i> UPDATE LABORATORY</a>
    </div>
  </div>
        ...
        <div class="widget-box">
            <div class="widget-title">
                <ul class="nav nav-tabs labell">
                    <li class="active"><a data-toggle="tab" href="#tab1">Update Lab Test</a></li>
                    ...
                    <form method="post" enctype="multipart/form-data" onsubmit="return confirm('CONFIRM UPDATE');">
                        <div class="span6">
                            <div class="widget-content nopadding">
                              <div class="control-group">
                                <label class="control-label">Lab Test ID :</label>
                               <div class="controls">
                                  <input type="text" class="span11" name="labID" value="<?php echo $flabrow['labID']; ?>" required readonly/>
                                </div>
                              </div>
                              <div class="control-group">
                                <label class="control-label">Lab Test Name :</label>
                               <div class="controls">
                                  <input type="text" class="span11" name="labName" placeholder="Lab Test Name" value="<?php echo $flabrow['labName'];?>" required/>
                                </div>
                        ...
                        <div class="widget-content nopadding">
                              <div class="form-actions">
                                  <i class="span5"></i>
                                <button type="submit" name="updatelab" class="btn btn-success labell btn-block span6">Update Lab Test</button>
                              </div>
                          </div>
